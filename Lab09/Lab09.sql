...
--5:
CREATE VIEW V_TeacherFail AS
SELECT Class.HeadTeacher
FROM Class JOIN Student ON Student.ClassCode = Class.ClassCode JOIN Mark ON Student.RollNo=Mark.RollNo
WHERE Mark.Mark < 8

--6:
CREATE VIEW V_6 AS
SELECT Student.FullName, Class.ClassCode, Class.HeadTeacher,Mark.Mark
FROM Student JOIN Class ON Student.ClassCode=Class.ClassCode JOIN Mark ON Mark.RollNo=Student.RollNo
WHERE Mark.SubjectCode = '005'
